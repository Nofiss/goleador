    ...
    <div className="space-y-6">
      ...
      <div className="rounded-md border bg-white shadow-sm">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
               <TableRow><TableCell colSpan={5} className="text-center h-24">Caricamento...</TableCell></TableRow>
            ) : tournaments?.length === 0 ? (
               <TableRow><TableCell colSpan={5} className="text-center h-24">Nessun torneo trovato.</TableCell></TableRow>
            ) : (
              tournaments?.map((t) => (
                <TableRow key={t.id}>
                  <TableCell className="font-medium">{t.name}</TableCell>
                  <TableCell>{t.type === TournamentType.RoundRobin ? "Girone" : "Eliminazione"}</TableCell>
                  <TableCell>{t.teamSize === 1 ? "1 vs 1" : "2 vs 2"}</TableCell>
