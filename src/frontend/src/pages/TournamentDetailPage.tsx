...
export const TournamentDetailPage = () => {
	...
	return (
		<div className="space-y-6">
				...
				<div className="text-xs text-gray-500 mb-6 flex gap-4 flex-wrap">
					...
					{tournament.scoringRules.goalThreshold && (
						<span className="bg-blue-50 px-2 py-0.5 rounded text-blue-700">
							⚽ Bonus: +{tournament.scoringRules.goalThresholdBonus}pt se{" "}
							{tournament.scoringRules.goalThreshold}+ goal
						</span>
					)}
			...
			<Tabs
				defaultValue={
					tournament.status === TournamentStatus.Active ? "matches" : "teams"
				}
			>
				<TabsList>
							...
							{tournament.teams.length === 0 ? (
								...
								<ul className="space-y-2">
									{tournament.teams.map((t) => (
										<li
											key={t.id}
											className="p-2 bg-gray-50 rounded border flex items-center gap-2"
										>
				...
				<TabsContent value="matches">
					{tournament.status === TournamentStatus.Setup ? (
						<Alert>
							...
							<AlertDescription>
								Avvia il torneo per generare il calendario.
							</AlertDescription>
						</Alert>
					) : (
						<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
							{tournament.matches.map((match) => (
								<div
									key={match.id}
									className="border rounded-lg p-4 bg-white shadow-sm flex flex-col items-center"
								>
