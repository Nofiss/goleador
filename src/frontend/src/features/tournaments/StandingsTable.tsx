...
export const StandingsTable = ({ tournamentId }: { tournamentId: string }) => {
	...
	if (isLoading)
		return <div className="text-center p-4">Caricamento classifica...</div>;
				...
				<TableBody>
					{standings?.length === 0 ? (
						...
						standings?.map((row, index) => (
							<TableRow
								key={row.teamId}
								className={cn(index === 0 && "bg-yellow-50/50")}
							>
								...
								<TableCell className="text-center hidden md:table-cell text-gray-400">
									{row.drawn}
								</TableCell>
								<TableCell className="text-center hidden md:table-cell text-red-400">
									{row.lost}
								</TableCell>
